...
warnings.filterwarnings("ignore", message=".*quota project.*", category=UserWarning, module="google.auth")
...
class AnalysisState(TypedDict):
    ...
    period: Optional[str]  # Period key for centralized config lookup
...
def _extract_pr_info(pr_item: dict) -> Optional[dict[str, int | str]]:
...
def _paginate_github_search(url: str, headers: dict[str, str], query: str, description: str) -> list[dict]:
    ...
    try:
        ...
        if filtered_reviews or filtered_comments:
            review_data["review_data"][str(pr_number)] = {
                ...
                "created_at": pr_details.get("created_at", "")[:10] if pr_details.get("created_at") else "",
...
def load_config(state: AnalysisState) -> AnalysisState:  # type: ignore[return]
...
def fetch_github_data(state: AnalysisState) -> AnalysisState:  # type: ignore[return]
    ...
    all_reviewed_prs = _paginate_github_search(url, headers, query_reviewed, "Searching for reviewed PRs")
    
    # Search for PRs authored by user
    query_authored = f"org:{organization} author:{username} type:pr created:{start_date}..{end_date}"
    all_authored_prs = _paginate_github_search(url, headers, query_authored, "Searching for authored PRs")
    ...
    total_authored = min(len(authored_pr_list), MAX_AUTHORED_PRS)
    ...
    if RICH_AVAILABLE and total_reviewed > 0:
        ...
        with Progress(
            ...
            task = progress.add_task(f"Fetching {total_reviewed} reviewed PRs...", total=total_reviewed)
            for pr_info in reviewed_pr_list[:MAX_REVIEWED_PRS]:
                _fetch_reviewed_pr_details(pr_info, headers, username, start_dt, end_dt, review_data)
...
def analyze_with_vertexai(state: AnalysisState) -> AnalysisState:
    ...
    if not project:
        state["error"] = "Vertex AI project required. Set GOOGLE_CLOUD_PROJECT environment variable."
        return state
...
def generate_report(state: AnalysisState) -> AnalysisState:  # type: ignore[return]
    ...
    prs = review_data.get("review_data", {})
    total_comments = sum(
        len(pr.get("reviews", [])) + len(pr.get("review_comments", []))
    ...
    template_context = {
        ...
        "conflict_management_examples": analysis.get("conflict_management_examples", "Examples pending"),
        "conflict_analysis": analysis.get("conflict_analysis", "Analysis pending"),
        "significant_changes": analysis.get("significant_changes", "No significant changes identified"),
...
def run(config_path: str, github_token: str = None, vertexai_project: str = None, vertexai_location: str = None, username: str = None, period: str = None, output_dir: str = None):
    """
